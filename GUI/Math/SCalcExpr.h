	...
	private:
		...
		*/
		struct readData_t
		{
			...
			irr::core::array<u32> parToRead;			//!< this is a stack of the expected number of parameters of functions in the expression
            //! Simple constructor with initialization
			readData_t() {init();};
		...
		irr::core::array<elm_t> postfixExpr;					//!< the expression in post fix notation
		...
		irr::core::array<stackElm_t> stck;	//!< Stack for parsing the expression
		irr::core::array<f32> fstck;		//!< Stack for calculating the expression
