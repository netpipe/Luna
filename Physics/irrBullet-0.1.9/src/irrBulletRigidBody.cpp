...
IRigidBody::IRigidBody(irrBulletWorld* const world, ICollisionShape* const collShape)
{
...
void IRigidBody::getAabb(vector3df &aabbMin, vector3df &aabbMax)
{
    ...
    aabbMin = bulletToIrrlichtVector(aabbMinBt);
    aabbMax = bulletToIrrlichtVector(aabbMaxBt);
}
