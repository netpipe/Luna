    ...
    ChopperControl::ChopperControl  (IrrlichtDevice* device, scene::ISceneNode* node )
     :       Device(device),
    ...
    void ChopperControl::checkKey()
    {
            if (m_cInGameEvents->getKeyState( KEY_KEY_W )) { this->pitchDown(); }
    ...
    void ChopperControl::setEventRec(InGameEventReceiver *Events){m_cInGameEvents = Events;}
    ...
    void ChopperControl::update()
    {
            ...
            if (IsResettingPitch)
            {
                    f32 t = (time - ResetPitchStartTime) * 0.0008f;
                    f32 interpolate = 1.0f - exp(- RATE_CONSTANT * t);

                    f32 pitchAngleDegs = (1.0f - interpolate) * OldPitchAngleDegs;
                  //  if (pitchAngleDegs > 1) {pitchAngleDegs = 1;}
    ...
    {
            f32 z = PitchEmpty->getRotation().X;
            if ((z>90.0f) && (z<270.0f))
