...
void Player::registerIrrlichtDevice(f32 frameDeltaTime2,IrrlichtDevice &device) {
	m_irrDevice = &device;
	frameDeltaTime = frameDeltaTime2;
}
...
void Player::moveForward()
{
  ...
  newPos = (facing * moveSpeed * frameDeltaTime) + playerNode->getPosition();
...
void Player::strafeLeft()
{
  ...
  facing.normalize();
  newPos = (facing*(moveSpeed/2)* frameDeltaTime) + playerNode->getPosition();
  playerNode->setPosition( newPos );
  playerNode->updateAbsolutePosition();
