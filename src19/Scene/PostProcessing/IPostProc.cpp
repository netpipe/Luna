...
IPostProc::IPostProc( irr::scene::ISceneManager* sm, irr::core::dimension2du sOut ) {
	smgr = sm;
	init( sOut );
}
IPostProc::IPostProc( IPostProc* follow1, irr::core::dimension2du sOut ) {
...
void IPostProc::init( irr::core::dimension2du sOut ) {
	irr::video::IVideoDriver* driver = smgr->getVideoDriver( );
	if( sOut.Width == 0 || sOut.Height == 0 )
		output = driver->addRenderTargetTexture( irr::core::dimension2du( 1, 1 ) );
	else
	...
	supported = driver->getDriverType( ) != irr::video::EDT_SOFTWARE && driver->getDriverType( ) != irr::video::EDT_BURNINGSVIDEO && driver->getDriverType( ) != irr::video::EDT_NULL;
...
void IPostProc::copyEffect( irr::video::ITexture* target ) {
	...
	if( supported && target != NULL )
		driver->setViewPort( irr::core::rect<irr::s32>( irr::core::position2d<irr::s32>( 0, 0 ), driver->getScreenSize( ) ) );
	driver->setRenderTarget( target, false, false );
...
void IPostProc::render( irr::video::ITexture* target, irr::u32 rid, bool p ) {
	...
	lastRender = rid;
	irr::video::IVideoDriver* driver = smgr->getVideoDriver( );
	if( (outs.size( ) > 1u || p || requireSelfRender) && supported && enabled ) {
		irr::core::rect<irr::s32> curSz = driver->getViewPort( );
		if( supported )
			driver->setViewPort( irr::core::rect<irr::s32>( irr::core::position2d<irr::s32>( 0, 0 ), driver->getScreenSize( ) ) );
		renderEffect( output );
