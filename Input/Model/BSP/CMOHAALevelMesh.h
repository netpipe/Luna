...
// based on
// CQ3LevelMesh.h Copyright (C) 2002-2011 Nikolaus Gebhardt
...
namespace scene
{
	class CMOHAALevelMesh : public IQ3LevelMesh
	{
	public:

		//! constructor
		CMOHAALevelMesh(io::IFileSystem* fs, scene::ISceneManager* smgr,
		             const quake3::Q3LevelLoadParameter &loadParam);

		//! destructor
		virtual ~CMOHAALevelMesh();
...
virtual IMesh* getBrushEntityMesh(quake3::IEntity &ent) const;
	...
	private:
		...
		enum eLumps
		{
			...
			kMeshVerts			= 5,	// Stores the model vertices offsets
			...
			kSideEquations		= 10,	// side equations
			...
			kLightGridPalette	= 16,	// Stores extra world lighting information
			kLightGridOffsets	= 17,	// Stores extra world lighting information
			kLightGridData		= 18,	// Stores extra world lighting information
			kSphereLights		= 19,	// sphere lights
			kSphereLightVis		= 20,	// sphere lights
			kDummy1				= 21,	// unknown data
			kTerrain			= 22,	// Terrain
			kTerrainIndexes		= 23,	// Terrain Indexes
			kStaticModelData	= 24,	// Static Mode lData
			kStaticModelDefs	= 25,	// Static Model Defs
			kStaticModelIndexes	= 26,	// Static Model Indexes
			kDummy2				= 27,	// unknown data
		...
		struct tBSPHeader
		{
			s32 strID;      // This should always be '2015'
			s32 version;    // This should be 0x13 for Quake 3 files
			s32 checksum;	// CRC of map?
		};
		tBSPHeader header;
		...
		typedef void( CMOHAALevelMesh::*tParserCallback ) ( quake3::SVarGroupList *& groupList, eToken token );
