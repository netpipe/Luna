...
SoundFactory::~SoundFactory() { flush(); }

void SoundFactory::setActive ( bool flag ) { active = flag; } //! Sets the sound factory as active
bool SoundFactory::isActive() { return active; } //! Checks to see if the sound factory is active

bool SoundFactory::loadOgg ( const std::string &soundName, const bool &intoMemory )
{
	OggFile *newOgg = new OggFile();
	if ( newOgg->open ( soundName, intoMemory ) < 0 )
	{
...
unsigned long SoundFactory::loadSound ( const std::string &soundFile, const bool &intoMemory)
{
	...
	if ( extension == "ogg" ) {
		if ( loadOgg ( soundName, intoMemory ) )
		{
			SoundSource *newSource = new SoundSource();
			while ( sources.find ( sourceRefCount ) != sources.end() || sourceRefCount == 0 )
				sourceRefCount++;
