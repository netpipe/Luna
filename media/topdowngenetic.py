...
#     aliveCount=0
#     if populationAlive:
#         for ind in genetic.individuals:
#             aliveCount+=int(ind.isAlive)
#         if aliveCount<=1:
...
#             dists=[]
#             inputData=[ind.pos[0],ind.pos[1],ind.direction,ind.angularVel,ind.speed[0],ind.speed[1]]
#             for bullet in bullets:
#                 dists.append(distance(ind.pos,bullet.pos))
#             dists.sort(key=lambda a : a[2])
#             if len(dists)>0:
#                 for i in range(2):
#                     if i<len(dists):
#                         inputData+=[dists[0][0],dists[0][1]]
#                     else:
#                         inputData+=[0,0]
#             else:
#                 inputData+=[0,0,0,0]
#             closest=None
#             for ind2 in genetic.individuals:
#                 if ind2.id!=ind.id:
#                     if closest is None:
#                         closest=ind2
#                     else:
#                         if distance(ind.pos,ind2.pos)<distance(ind.pos,closest.pos):
#                             closest=ind2
#             inputData+= [closest.pos[0],closest.pos[1],closest.direction,closest.angularVel]
#             ind.updateNN([inputData])
#         sch=[]  
...
#             for individual in genetic.individuals:
#                 if checkCollision(individual,bullets[i]):
#                     individual.damage(bullets[i].damage)
#                     bullets[i].isAlive=False
#                     for indivi in genetic.individuals:
#                         if indivi.id == bullets[i].id:
#                             indivi.damageDone+=20
