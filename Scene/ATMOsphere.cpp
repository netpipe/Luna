...
#include <math.h>
#include <ITimer.h>
//#include "flares/LensFlareSceneNode.h"
...
//    scene::LensFlareSceneNode *flare;
...
video::SColorf(1.0f, 0.6f, 0.7f, 1.0f), 10000.0f);
 ...
 void start(IrrlichtDevice* device2,video::IVideoDriver* Driver,scene::ISceneNode* parent, scene::ISceneManager* mgr, s32 id)
 {
  ...
  setAmbientLight2(SColor(255,255,255,255));//bug fix
     ...
     //   flare = new scene::CLensFlareSceneNode(NULL, smgr, -1, core::vector3df(0,110,0));
     //   flare->getMaterial(0).setTexture(0, smgr->getVideoDriver()->getTexture("media/flares.jpg"));
    //     flare->setposition( core::vector3df(0,5500,0));
 }
 ...
 vector3df getsunpos(){
  return getInterpolated3df(sun_pos_from,sun_pos_to, time_int_step);// angle in radians
 }
...
//     sunMeshNode->setPosition(vt);

  //  lensFlareNode->render();

//maybe put this somewhere else to speed things up do more at once type thing
