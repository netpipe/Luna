...
CLensFlarePostProc::CLensFlarePostProc( IPostProc* follow, irr::core::dimension2du sOut, irr::video::ITexture* m, irr::core::vector3d<irr::f32> p, irr::f32 r ) : IPostProc( follow, sOut ) {
	setPosition( p, r );
	init( follow, sOut, m );
}
CLensFlarePostProc::CLensFlarePostProc( IPostProc* follow, irr::core::dimension2du sOut, irr::video::ITexture* m, irr::scene::ISceneNode* p, irr::f32 r ) : IPostProc( follow, sOut ) {
	setPosition( p, r );
	init( follow, sOut, m );
}
...
void CLensFlarePostProc::init( IPostProc* follow, irr::core::dimension2du sOut, irr::video::ITexture* m ) {
...
void CLensFlarePostProc::renderEffect( irr::video::ITexture* target ) {
	...
	irr::core::dimension2du dsc = driver->getScreenSize( );
	if( pS.X < 0 || pS.Y < 0 || pS.X >= dsc.Width || pS.Y >= dsc.Height )
		return;

	irr::core::dimension2du dim = tex->getSize( );
